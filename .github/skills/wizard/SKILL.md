...
A wizard is ephemeral by default — built for one run, saved to a scratch or `.github/skills/.skillbridge/packs/mattpocock__skills/scripts/` path, deleted when the job's done. Commit it only when the user wants a repeatable setup path that should live in the repo.
