...
Installed command: `/diagnosing-bugs`.

Use it in VS Code Chat / Copilot Agent Mode by typing:

`/diagnosing-bugs <your arguments>`
...
10. **HITL bash script.** Last resort. If a human must click, drive _them_ with `.github/skills/.skillbridge/packs/mattpocock__skills/scripts/hitl-loop.template.sh` so the loop is still structured. Captured output feeds back to you.
...
- [ ] **Agent-runnable** — you can run it unattended; a human in the loop only via `.github/skills/.skillbridge/packs/mattpocock__skills/scripts/hitl-loop.template.sh`.
