...
import { projectRoot, resolveInProject, textResult, truncate } from "../helpers.js";
...
export function registerFileTools(server: McpServer): void {
  server.tool(
    ...
    {
      path: z.string().describe("File path relative to the project root, e.g. prompts/linkedin-job-assistant.system.md"),
    },
  ...
  server.tool(
    ...
    {
      path: z.string().optional().describe("Directory relative to project root; defaults to the project root."),
    },
    async ({ path: inputPath = "." }) => {
      try {
        const resolved = resolveInProject(inputPath);
  ...
  server.tool(
    ...
    {
      pattern: z.string().describe("Regular expression to search for, e.g. 'external.*confirm'"),
      path: z.string().optional().describe("Directory to search, relative to project root; defaults to the project root."),
        ...
        for (const file of files) {
          ...
          for (let i = 0; i < lines.length && matches.length < 100; i++) {
            if (regex.test(lines[i])) {
              matches.push(`${path.relative(projectRoot, file)}:${i + 1}: ${lines[i].trim()}`);
            }
          }
        }
  ...
  server.tool(
    ...
    async ({ path: inputPath, content }) => {
      try {
        ...
        return textResult(`[write_file] wrote ${content.length} chars to ${inputPath}`);
      } catch (error) {
  ...
  server.tool(
    ...
    {
      path: z.string().describe("File path relative to the project root"),
      old_string: z.string().describe("Exact text to find (including whitespace)"),
      new_string: z.string().describe("Replacement text"),
      allow_multiple: z.boolean().optional().describe("Allow replacing every occurrence when old_string appears more than once"),
    },
    async ({ path: inputPath, old_string, new_string, allow_multiple = false }) => {
      try {
        const resolved = resolveInProject(inputPath);
        const current = await readFile(resolved, "utf8");

        if (old_string.length === 0) {
          return textResult("[edit_file] error: old_string must not be empty", true);
        }
