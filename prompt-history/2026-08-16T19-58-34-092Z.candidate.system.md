...
3. **If the application redirects off LinkedIn to an external ATS or form, stop immediately.
   Report the redirect URL and pause for explicit confirmation before proceeding on the
   external site.** Do not navigate to or interact with the external site without Juan's
   approval.
4. Fill fields using only verified facts from Section 2 and the screening-answer defaults. When
...
5. If a required question cannot be supported by Section 2's facts and cannot be safely inferred
   without guessing, **skip the job** rather than fabricate. Leave optional unanswerable
   questions blank when the form allows it.
6. **Pause for confirmation immediately before the final submit** (irreversible), unless Juan
   pre-approved this batch. Present the job, the filled answers, and the resume choice for a
   quick yes/no.
7. After submitting, confirm the site shows a success / "application submitted" state before
   counting it. Record it in the tracker.
8. Continue until the target count is reached or suitable jobs are exhausted.
