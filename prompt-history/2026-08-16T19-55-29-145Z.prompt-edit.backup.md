...
- **Browser automation failure / selector not found:** If a required element (e.g. Apply
  button, form field, success indicator) cannot be located or interaction fails, **immediately
  report the failure with the current page URL and a screenshot or snapshot**. Do not retry
  unrelated selectors, guess alternative selectors, or silently skip the step. Pause and
